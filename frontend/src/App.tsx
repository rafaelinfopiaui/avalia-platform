...
function SessionGuard() { const { user, loading } = useAuth(); const location = useLocation(); const navigate = useNavigate(); useEffect(() => { const handler = () => navigate('/login', { replace: true, state: { from: location.pathname } }); window.addEventListener('avalia:unauthorized', handler); return () => window.removeEventListener('avalia:unauthorized', handler) }, [location.pathname, navigate]); if (loading) return <main className="center-page"><Spinner label="Verificando sessão…" /></main>; return user ? <Outlet /> : <Navigate to="/login" replace state={{ from: location.pathname }} /> }
